...
/** @example justPathPlanningGuiServer.cpp Example which does path planning and
...
main(int argc, char *argv[])
{
  ...
  if (!sick.blockingConnect())
    ArLog::log(ArLog::Normal, "Couldn't connect to SICK laser, it won't be used");
  else
  ...
  */
  ...
  {
    ArLog::log(ArLog::Normal, "");
    ArLog::log(ArLog::Normal, "### Warning, No map file is set up, you can make a map with sickLogger or guiServer, and/or Mapper3; More info in docs/Mapping.txt and README.txt. Set the map with the -map command line option, or by changing the config with MobileEyes or by editing the config file.");
    ArLog::log(ArLog::Normal, "");
  }
